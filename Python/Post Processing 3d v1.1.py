...
def animate(n):
    ...
    for i in bodyList:
        ...
        if n > l:   
            ax.plot(x[n-l:n],y[n-l:n],z[n-l:n],'--', lw = 2)
        else:
            ax.plot(x[:n],y[:n],z[:n],'--', lw = 2)
        ax.scatter(x[n],y[n],z[n], s = MAX_PLANET_SIZE * i.GetMass(n) / 100000, c = i.GetColor())
        ax.grid(False)
...
#anim.save('C:\Users\Andy\Desktop\Programs\C ++\Projects\N-body Sim\NBodySim v1.5\Nbody v1.5\Test2.MP4',fps = 25,bitrate = 4500)
