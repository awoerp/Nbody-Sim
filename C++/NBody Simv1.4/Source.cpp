...
public:
   ...
   void PrintPlottingData()
   {
      ...
      for(int i = 0; i < numBodies; i++)
      {
         if(list[i].IsActive())
         {
            float Px = list[i].GetPx();
            float Py = list[i].GetPy();
            float mass = list[i].GetMass();
            totalMass += mass;
      ...
      centerOfMassX = sumX / totalMass;
      centerOfMassY = sumY / totalMass;
      ...
      for(int i = 0; i < numBodies; i++)
      {
         float Px = list[i].GetPx();
         float Py = list[i].GetPy();
         sumX += pow(Px - centerOfMassX, 2);
         sumY += pow(Py - centerOfMassY, 2);
      }
      deviationX = sqrt(sumX / numBodies);
      deviationY = sqrt(sumY / numBodies);

      outputFile << centerOfMassX << " "
                 << centerOfMassY << " "
                 << deviationX    << " "
                 << deviationY    << endl;
      ...
      for(int i = 0; i < numBodies; i++)
      {
         float Px = list[i].GetPx();
         float Py = list[i].GetPy();
         bool inRegionX = Px < (centerOfMassX +  3*deviationX) &&
                          Px > (centerOfMassX -  3*deviationX);
         bool inRegionY = Py < (centerOfMassY +  3*deviationY) &&
                          Py > (centerOfMassY -  3*deviationY);
